...
let main argv =
    ...
    printfn "type Person example '%A'" { Name = "Bill" ; DateOfBirth = DateTime(1962, 09, 02) } 
    printfn "stats function for Google example '%A'" (stats "www.google.com")  
    printfn "isVowel function example '%A'" (isVowel 'f') 

// Generic Monadic Tree - let's try RoseTree with int 
    let n1 = Knob (10, NoRose) 
    let n2 = Knob (20, NoRose) 
    let n3 = Knob (20, Rose (n1, NoRose)) 
    printfn "RoseTree int example '%A'" n3 
...
// let's try RoseTree with Transport 
    ...
    printfn "RoseTree Transport example '%A'" t6 
...
// let's try RoseForest with char   
    ...
    printfn "RoseForest char example '%A'" rt1 

// comparison examples 
    printfn "compare ('a','b') < ('a','z') example '%A'" (('a','b') < ('a','z'))  
    printfn "compare ('a','b') ('a','z') example '%A'" (compare ('a','b') ('a','z')) 
    printfn "compare [10; 30] [10; 20] example '%A'" (compare [10; 30] [10; 20]) 
    printfn "compare [|10; 30|] [|10; 20|] example '%A'" (compare [|10; 30|] [|10; 20|]) 
    printfn "compare [|10; 30|] [|10; 40|] example '%A'" (compare [|10; 30|] [|10; 40|] ) 

// hash examples 
    printfn "hash 100 example '%A'" (hash 100) 
    printfn "hash abc example '%A'" (hash "abc") 
    printfn "hash (100, abc) example '%A'" (hash (100, "abc")) 

// generic boxing and unboxing 
// this is used to convert data to obj type used by dotnet C# 
    printfn "box 1 example '%A'" (box 1) 
    let stringObj = box "abc"  
// must tell unbox what the underlying type really is ... 
    printfn "(unbox<string> stringObj) example '%A'" (unbox<string> stringObj) 
    printfn "(unbox stringObj : string) example '%A'" (unbox stringObj : string)   
// this fails with System.InvalidCastException
// (unbox stringObj : int)  

// serialize example 
    let addresses = 
        Map.ofList [("Jeff", "123 Main st Red WA 98052") ; ("Fred", "987 Pind rd Phila PA 19116") ; ("Mary", "PO Box 123, Palo Alto CA 94301") ]
    ...
    printfn "res example '%A'" res 
    fsIn.Close() 

// hcf 
    printfn "hcfInt 18 12 example '%A'" (hcfInt 18 12) 
    printfn "hcfbigInt 1810287116I 1239056I example '%A'" (hcfbigInt 1810287116I 1239056I)
...
// sub-typing 
    ...
    printfn "downcastString example '%A'" downcastString 

    let xobj2 = box 1  
    let x2 = (xobj2 :? string)  
// System.InvalidCastException: 
// let x3 = (xobj2 :?> string)  
    printfn "checkObject (box 1) example '%A'" (checkObject (box 1)) 
    printfn "checkObject (box abc) example '%A'" (checkObject (box "abc")) 
    printfn "checkObject (box 10.5) example '%A'" (checkObject (box 10.5)) 

    printfn "reportObject (box 1) example '%A'" (reportObject (box 1)) 
    printfn "reportObject (box abc) example '%A'" (reportObject (box "abc")) 
    printfn "reportObject (box 10.5) example '%A'" (reportObject (box 10.5))

    printfn "All finished from ExpertF#Ch05" 
